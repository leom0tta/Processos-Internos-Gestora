...
def main():
    ...
    try:
        ...
        if not portfolios:
            print("Erro: Nenhum portfolio encontrado!")
            sys.exit(1)

        print(f"\n{len(portfolios)} portfolio(s) encontrado(s):")
        for idx, port in enumerate(portfolios[:10], 1):
            print(f"  {idx}. {port.get('name', f'Portfolio {idx}')}")

        opcao = input("\nEscolha o número do portfolio: ").strip()
        portfolio_id = portfolios[int(opcao) - 1]['id']
        ...
        if not posicoes_proc:
            print("Erro: Nenhuma posição processada!")
            sys.exit(1)

        print(f"\n⏳ Calculando alocações...")
        alocacoes, patrimonio_total = system.calcular_alocacoes(posicoes_proc)

        if patrimonio_total == 0:
            print("Erro: Patrimônio total zerado!")
            sys.exit(1)

        print(f"Patrimônio total: R$ {patrimonio_total:,.2f}".replace(',', '.'))

        print(f"\n⏳ Analisando suitability...")
        analise_suit = system.analisar_suitability(alocacoes, perfil)

        print(f"\n⏳ Gerando laudo...")
        output_file = system.gerar_docx(
        ...
        print(f"\n✅ Laudo gerado com sucesso: {output_file}")
